...
def predict_quad(model, img, pixel_threshold=cfg.pixel_threshold, quiet=False, img_name=None):
    ...
    img = img.resize((d_wight, d_height), Image.BICUBIC).convert('RGB')
    ...
    for n in range(num_img):
        ...
        # 功能为从第n张图片的预测返回值中找满足阈值条件的像素点实施nms
        # (sample, rows, cols, 7_points_pred)
        # 返回第n个sample， y=（rows，cols，7_points_pred）
        y = y_pred[n]
        # predict[0/1/2]经过sigmoid函数
        # 0：是否为内部像素 1：是否为边界像素 2：头或尾边界
        y[:, :, :3] = sigmoid(y[:, :, :3])
        cond = np.greater_equal(y[:, :, 0], pixel_threshold)
        # where函数返回满足给定条件的元素索引
        ...
        with image.array_to_img(img_all[n]) as im:     # Image.fromarray(x[n]) error ?
            ...
            if True:
                # print('rec个数为%d'%(len(quad_after_nms)))
                # print(quad_scores)
                for score, geo, s in zip(quad_scores, quad_after_nms,
                    ...
                    if np.amin(score) > 0:
                        ...
                        # valid_text_recs.append(np.reshape(geo,(8,)).tolist())
                                    
                        if cfg.predict_cut_text_line: 
                            cut_text_line(geo, scale_ratio_w, scale_ratio_h, im_array,img_name, s)
